...
impl GitObject {
    ...
    pub fn from_path(path: impl AsRef<Path>, write: bool) -> Result<Self, GitError> {
        //path to read the files/folders from
        ...
        if path.is_file() {
            ...
            if write {
                //this function is used to write a blob object created into the .rvc/objects/<[..2]>/<[2..]>
                git_object.write()?;
            }
        ...
        if path.is_dir() {
            ...
            for (filename, mode) in entries {
                //here we are checking each entry recursively is file then it is returned otherwise again traversal of that directory is done
                 let git_object = match Self::from_path(path.join(&filename), write) {
                    Ok(obj) => obj,
                    Err(e) => {
                        println!("Skipping {} due to error: {:?}", filename, e);
                        continue;
                    }
                };
            ...
            if write {
                //storing the objects in .rvc/objects
                git_object.write()?;
            }
            return Ok(git_object);
        }
